...
if($use_uploaded_logo)
{
	...
	{
		echo '<style>.logo-image { width: '.$custom_logo_max_width.'px; }</style>';
	}
}
?>
...
<?php if($use_uploaded_logo && isset($logo_image_url)): ?>
	<img src="<?php echo esc_url( str_replace( array( 'http:', 'https:' ), '', $logo_image_url ) ); ?>" width="<?php echo $logo_image[1]; ?>" height="<?php echo $logo_image[2]; ?>" alt="logo" />
<?php
else:
