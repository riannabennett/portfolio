...
if( preg_match_all( "/^([0-9]+)x?([0-9]+)?x?(0|1)?$/", $shop_category_image_size, $shop_category_image_dims ) )
{
	...
	$shop_category_thumb_crop      = boolval( $shop_category_thumb_crop[3][0] );
			...
			elseif( $item_preview_type == 'gallery' && ! empty( $product_images ) ) :
				...
				foreach( $product_images as $attachment_id ) :
					...
					if( $attachment_id != $post_thumb_id ) :
						...
						?>
						<a href="<?php the_permalink(); ?>" class="product-gallery-image" data-index="<?php echo $index; ?>">
...
function lab_wc_shop_products_container_after()
{
	...
	?>
		...
		<div class="col-md-3">
			...
			<div class="blog-sidebar shop-sidebar shop-sidebar-<?php echo $shop_sidebar; ?>">
...
function lab_wc_woocommerce_form_field_args( $args )
{
	...
	if( in_array( $args['type'], array( 'text', 'password', 'state', 'country' ) ) )
	{
...
function lab_wc_cart_menu_icon( $skin )
{
	...
	?>">
		...
		<a href="<?php echo WC()->cart->get_cart_url(); ?>" class="cart-icon-link icon-type-<?php echo $icon; ?>">
			<i class="icon-<?php echo $icon; ?>"></i>
			
			<?php if( $cart_items_counter ): ?>
			<span class="items-count<?php when_match($ajax_mode, 'hide-notification'); ?> cart-items-<?php echo $cart_items; ?>"><?php echo $ajax_mode ? '0' : $cart_items; ?></span>
			<?php endif; ?>
		</a>
...
function lab_wc_cart_menu_icon_mobile()
{
	...
	<div class="cart-icon-link-mobile-container">
		<a href="<?php echo WC()->cart->get_cart_url(); ?>" class="cart-icon-link-mobile icon-type-<?php echo $icon; ?>">
			<i class="icon icon-<?php echo $icon; ?>"></i>
			
			<?php _e( 'Cart', 'kalium' ); ?>
			
			<?php if( $cart_items_counter ): ?>
			<span class="items-count<?php when_match($ajax_mode, 'hide-notification'); ?> cart-items-<?php echo $cart_items; ?>"><?php echo $ajax_mode ? '...' : $cart_items; ?></span>
			<?php endif; ?>
		</a>
	</div>
	<?php
