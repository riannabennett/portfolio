...
?>
<nav class="woocommerce-pagination pagination-holder <?php echo "pagination-holder-{$shop_pagination_position}"; ?><?php echo $shop_catalog_layout != 'default' || ( $shop_catalog_layout == 'default' && get_data( 'shop_loop_masonry' ) ) ? ' no-top-margin' : ''; ?>">
	<?php
		# start: modified by Arlind Nushi
		if( in_array( $shop_pagination_type, array( 'endless', 'endless-reveal' ) ) ) :
			...
			$endless_opts = array(
				...
				'opts'         => array(
					'q' => rot13encrypt(serialize($wp_query->query))
				)
			);
