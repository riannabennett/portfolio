...
<div class="<?php echo implode(" ", $item_class); ?>">
	...
	<div class="member do-lazy-load-on-shown<?php echo esc_attr($css_class); echo $wow_effect ? esc_attr(" wow {$wow_effect}") : ""; ?>" data-wow-duration="1s"<?php if($wow_one_by_one): ?> data-wow-delay="<?php echo esc_attr($wow_delay); ?>s"<?php endif; ?>>
		<div class="thumb">
			<?php if($hover_style != 'none'): ?>
			<div class="hover-state padding<?php when_match($hover_style == 'distanced', 'hover-style-one'); ?>">
				<?php if($description): ?>
				<div class="social">
					<?php echo lab_esc_script($description); # escaped by ACF plugin ?>
				</div>
				<?php endif; ?>
